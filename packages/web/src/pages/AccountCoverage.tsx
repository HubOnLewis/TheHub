  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 12, display: 'grid', gridTemplateColumns: '1fr 200px 180px 180px auto auto auto', gap: 8, alignItems: 'end' }}>
        ...
        <label style={{ fontSize: 12 }}><input type="checkbox" checked={hasOpenDeals === 1} onChange={e => setSp(prev => { if (e.target.checked) prev.set('hasOpenDeals', '1'); else prev.delete('hasOpenDeals'); return prev; })} /> Open deals</label>
          ...
          {data.map(r => (
            <div key={r.companyId} className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                ...
                <Link className="btn btn-secondary" to={`/companies/${r.companyId}`}>Open Account</Link>
              </div>
              <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(5, minmax(120px, 1fr))', gap: 8 }}>
                ...
                <div className="stat-card"><div className="stat-label">Open/Active Deals</div><div className="stat-value">{r.accountPenetrationState.openDeals}/{r.accountPenetrationState.activeDeals}</div></div>
