...
export abstract class BaseRepository<TDoc extends Document> {
  ...
  async list(
    ...
    const [docs, total] = await Promise.all([
      this.col(db).find(scopedFilter).sort({ [options.sort]: sortDir }).skip(skip).limit(options.limit).toArray(),
  ...
  async insertOne(db: Db, ctx: TenantContext, payload: Omit<TDoc, '_id'>): Promise<TDoc & { _id: string }> {
    const doc = {
      ...payload,
      tenantId: ctx.tenantId ?? (payload as Record<string, unknown>)['tenantId'],
    };
    const { insertedId } = await this.col(db).insertOne(doc as unknown as OptionalUnlessRequiredId<TDoc>);
    return { ...doc, _id: insertedId.toString() } as unknown as TDoc & { _id: string };
  }

  async updateOne(db: Db, ctx: TenantContext, id: string, update: Partial<TDoc>): Promise<(TDoc & { _id: string }) | null> {
    let oid: ObjectId;
    try { oid = new ObjectId(id); } catch { return null; }
  ...
  async deleteOne(db: Db, ctx: TenantContext, id: string): Promise<boolean> {
    ...
    const { deletedCount } = await this.col(db).deleteOne(this.scope(ctx, { _id: oid } as Filter<TDoc>));
