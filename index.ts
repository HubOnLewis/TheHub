...
export function resolveTenant(req: Request, _res: Response, next: NextFunction): void {
  ...
  if (isSuperAdmin) {
    // Super admins see everything unless they explicitly scope
    tenantId = req.headers['x-tenant-override'] as string | null ?? null;
  } else if (isCrossTenant) {
    // Management can scope to a specific tenant via header
    const override = req.headers['x-tenant-override'] as string | undefined;
    tenantId = override ?? null; // null = see all tenants for their entity
  }
  // else: regular users are always hard-scoped to their own tenantId
